#!/usr/bin/env python3
"""
Add manual transcripts to Week 5 compiled document.
...
Then run this script to update week5_transcripts.md

Usage:
    python add_week5_manual_transcripts.py
"""
...
def find_week5_manual_transcripts(directory='.'):
    """Find all Week 5 manual transcript files"""
    manual_files = {}
...
def update_week5_document(manual_transcripts, compiled_file='week5_transcripts.md'):
    """Update the Week 5 compiled document with manual transcripts"""

    if not os.path.exists(compiled_file):
        print(f"Error: {compiled_file} not found!")
        print("Run process_week5_transcripts.py first to create the base document.")
        return False
    ...
    if updates_made > 0:
        ...
        print(f"  Make sure:")
        print(f"  - Student names in filenames match names in week5_transcripts.md")
        print(f"  - Files end with '_manual.txt'")
        print(f"  - Example: Harshini_Sarojini_manual.txt")
        print(f"{'='*80}")
        return False

def main():
    print("="*80)
    print("WEEK 5 MANUAL TRANSCRIPT PROCESSOR")
    print("="*80)
    print()

    # Find all Week 5 manual transcript files
    manual_transcripts = find_week5_manual_transcripts()
    ...
    if not manual_transcripts:
        ...
        print("  Allysse_Stokes_manual.txt")
        return
    ...
    # Process and update
    update_week5_document(manual_transcripts)
