#!/usr/bin/env python3
"""
Add manual transcripts to Week 2 AI Tool Use compiled document.

For students whose videos didn't have auto-captions, manually copy
the transcript and save as: StudentName_manual.txt

Then run this script to update week2_ai_tool_use_transcripts.md

Usage:
    python add_week2_manual_transcripts.py
"""
...
def find_week2_manual_transcripts(directory='.'):
    """Find all Week 2 manual transcript files"""
    manual_files = {}
...
def update_week2_document(manual_transcripts, compiled_file='week2_ai_tool_use_transcripts.md'):
    """Update the Week 2 compiled document with manual transcripts"""

    if not os.path.exists(compiled_file):
        print(f"Error: {compiled_file} not found!")
        print("Run process_week2_transcripts.py first to create the base document.")
        return False
    ...
    if updates_made > 0:
        ...
        print(f"  - Example: Charles_Barr_manual.txt")
        print(f"{'='*80}")
        return False

def main():
    print("="*80)
    print("WEEK 2 MANUAL TRANSCRIPT PROCESSOR")
    print("="*80)
    print()

    # Find all Week 2 manual transcript files
    manual_transcripts = find_week2_manual_transcripts()
    ...
    if not manual_transcripts:
        ...
        print()
        print("Examples:")
        print("  Charles_Barr_manual.txt")
        print("  Emma_Frank_manual.txt")
        return
    ...
    # Process and update
    update_week2_document(manual_transcripts)
